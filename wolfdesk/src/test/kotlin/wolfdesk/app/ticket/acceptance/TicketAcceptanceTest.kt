...
class TicketAcceptanceTest(
    ...
    Given("티켓을 여러개 생성하면") {
        ...
        티켓생성(jwt, command1) status HttpStatus.CREATED
        티켓생성(jwt, command2) status HttpStatus.CREATED

        Then("티켓 목록을 조회할 수 있다.") {
            val response = 티켓목록조회(jwt) status HttpStatus.OK
    ...
    Given("티켓을 생성하고") {
        ...
        val location = 티켓생성(jwt, command).header(HttpHeaders.LOCATION)
        var messageId = ""

        When("티켓에 메시지를 입력하면") {
            val messageCommand = createMessageAddCommandFixture()
            메시지등록(jwt, location, messageCommand) status HttpStatus.OK

            Then("메시지를 확인할 수 있다.") {
                val response = 티켓상세조회(jwt, location) status HttpStatus.OK
